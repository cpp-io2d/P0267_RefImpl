	...
	inline namespace v1 {
		namespace _Cairo {
			...
			inline typename _Cairo_graphics_surfaces<GraphicsMath>::surfaces::unmanaged_output_surface_data_type _Cairo_graphics_surfaces<GraphicsMath>::surfaces::create_unmanaged_output_surface(_UnmanagedSurfaceContext &context, int preferredWidth, int preferredHeight, io2d::format preferredFormat, io2d::scaling scl) {
				...
				data.hInstance = context.hInstance;
				data.hwnd = context.hwnd;
				data.hdc = context.hdc;
				// WNDCLASS names can be a max of 256 chars and needs an extra char for null terminator, hence 257.
				WCHAR wndClassName[257] = {};
				if (GetClassName(context.hwnd, wndClassName, ARRAYSIZE(wndClassName)) == 0) {
					_Throw_system_error_for_GetLastError(GetLastError(), "Failed call to GetClassName.");
				}
				WNDCLASS wndClass = {};
				if (GetClassInfo(context.hInstance, wndClassName, &wndClass) == 0) {
				...
				RECT clientRect = {};
				if (GetClientRect(context.hwnd, &clientRect) == 0) {
