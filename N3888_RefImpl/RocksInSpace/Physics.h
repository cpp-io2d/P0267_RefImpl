...
namespace asteroids
{
	...
	{
	public:
		controllable_physics(const physics& phy, const acc& acc, float ori) : m_physics(phy), m_acceleration(acc), m_orientation(ori) {}
		void		update();
		void		thrust(float t) { m_acceleration = pol_to_car(polar_2d{ t, m_orientation }); }
		void		spin(float spin) { m_orientation += spin; }
