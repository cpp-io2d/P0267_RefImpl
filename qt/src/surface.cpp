...
: _Lock_for_device()
, _Device()
, _Surface(unique_ptr<QPaintDevice>(nh.csfce))
, _Context(unique_ptr<QPaintEngine>(((nh.csfce == nullptr) ? nullptr : nh.csfce->paintEngine()))) {
}
...
: _Lock_for_device()
, _Device()
, _Surface(unique_ptr<QPaintDevice>(new QOpenGLPaintDevice(width, height)))
, _Context(unique_ptr<QPaintEngine>(_Surface.get()->paintEngine())) {
}

surface::surface(const surface& target, const rectangle& rect)
: _Lock_for_device()
, _Device()
, _Surface(unique_ptr<QPaintDevice>(new QOpenGLPaintDevice(rect.width, rect.height)))
, _Context(unique_ptr<QPaintEngine>(_Surface.get()->paintEngine())) {
}
...
void surface::set_antialias(antialias a) {
    cairo_set_antialias(_Context.get(), _Antialias_to_QFontOptions(a));
}

antialias surface::get_antialias() {
    return _QFontOptions_to_antialias(cairo_get_antialias(_Context.get()));
}
